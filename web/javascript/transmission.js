...
/** SAMS**/
		...
		$('#context_add_tracker').click($.proxy(this.addTrackerclicked,this));
		$('#add_confirm_button').click($.proxy(this.confirmAddClicked,this));
		$('#add_cancel_button').click($.proxy(this.hideAddDialog,this));
/** SAMS**/
	...
	****/
	...
	/*SAMS	*/
	addTrackerclicked: function() {
		this.addTracker(false);
		},


	confirmAddClicked: function() {
		this.addTracker(true);
		this.hideAddDialog();
	},

	hideAddDialog: function() {
		$('#tracker_container').hide();
		this.updateButtonStates();
	},
	...
	addTracker: function(confirmed) {
		var torrents = this.getSelectedTorrents();
		if (!confirmed)
		{
			$("#tracker_container").show();
		}
		else
		{
			var ids=this.getTorrentIds(torrents);
			var trackers=$("#trackers").val().split("\n");
			this.remote.addTtracker(ids,trackers,this.refreshTorrents,this);
			$('#tracker_container').hide();
		}
	},
